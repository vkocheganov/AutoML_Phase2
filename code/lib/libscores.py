...
def r2_metric(solution, prediction, task='regression'):
    ...
    mse = mvmean((sol-pred)**2)
    var = mvmean((sol-mvmean(sol))**2)
    score = 1 - mse / var
    return score
...
def a_metric (solution, prediction, task='regression'):
    ...
    sol=solution.ravel()
    pred=prediction.ravel()
    mae = float(mvmean(abs(sol-pred)))         # mean absolute error
    mad = float(mvmean(abs(sol-mvmean(sol)))) # mean absolute deviation
    return 1 - mae / mad
...
def show_all_scores(solution, prediction):
    ''' Show all the scores for debug purposes'''
